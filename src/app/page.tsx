...
export default function Home() {
  ...
  const addButtonClicked = () => {
    if (!addAmountField || isNaN(addAmountField) || addAmountField <= 0) {
    ...
    setAmountInWallet((prevAmount) => {
      const updatedAmount = (prevAmount ?? 0) + Number(addAmountField);
      return parseFloat(updatedAmount.toFixed(2)); // Round to 2 decimals
    });
  ...
  return (
    <>
      ...
      <div className="mt-10 h-10 w-44 rounded-lg flex justify-center items-center
              sm:mt-10 sm:h-12 sm:w-56 sm:rounded-lg 
              md:mt-8 md:h-14 md:w-56 md:rounded-lg 
              lg:mt-3 lg:h-12 lg:w-56 lg:rounded-lg 
